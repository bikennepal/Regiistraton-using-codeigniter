...
$this->form_validation->set_error_delimiters('<div class="text-danger">', '</div>');
  if($this->form_validation->run())
  {
      ...
      {
          $this->session->set_flashdata('user','successfully added a new user');
          $this->session->set_flashdata('user_class','alert-sucess');
      }
      else
      {
      	$this->session->set_flashdata('user','please try again');
